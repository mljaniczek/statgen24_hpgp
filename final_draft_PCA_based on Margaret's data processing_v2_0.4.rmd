...
As we can see there are 231 subjects missing values for astygmatism. I am going to exclude them from our analysis dataset. Additionally, there were 14 subjects that were missing >40% of SNP data so they will be excluded as well. This results in a sample size of 384 subjects.
...
# make index for if subject is missing more than 40% of SNP data
index_row <- rowna_percent >.4

index_cols <- colMeans(is.na(snps)) >.75  # there were no SNPs that were missing more than 75% of data
...
Next I calculated the percent of SNPs which had missingness. No SNPs were missing >75% of values, so we didn't need to filter out any SNPs for missingness. Finally, calculated the minor allele frequency (MAF) for all SNPs and excluded SNPs with MAF <5% for a final count of 4787 SNPs.
...
And below is the distribution of remaining missingness in 384 subjects for 4787 SNPs:
...
df_out2<- cbind(df_out,covars$Myopia)
names(df_out2)[387]<-"Myopia"

ggplot(df_out2, aes(x = PC1, y = PC2,
                   color = factor(Myopia), alpha = 0.6)) +
  geom_point() +
  theme_bw() +
  ggtitle("PCA PC1 vs PC2") +
  labs(color = "Myopia") +
...
```{r}
## select the 216 significant SNPs
...
df_out$gender <- as.factor(covars$Gender)
df_out$astigmatism <- as.factor(covars$Astigmatism)
df_out$Myopia <- as.factor(covars$Myopia)
...
covars_pcs$num_gender<-ifelse(covars_pcs$Gender=='Male',1,
                              ifelse(covars_pcs$Gender=='Female',0,NA))


# interaction between gender and myopia
covars_pcs$intera_gen_myo<-covars_pcs$num_gender*covars_pcs$Myopia

covars_pcs<-covars_pcs[,-24]
...
```{r}
# only having the interaction between gender and myopia
covars_pcs2<-covars_pcs[,-c(14:23)]

model_logit4 <- glm(Astigmatism ~ .,
  data = covars_pcs2,
  family = binomial(link = "logit")
)
